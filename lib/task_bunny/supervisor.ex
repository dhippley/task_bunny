  ...
  def init([wsv_name, ps_name]) do
    ...
    connections =
      Enum.map(
        Config.hosts(),
        fn host ->
          Supervisor.child_spec({Connection, host}, id: make_ref())
        end
      )
    ...
    children =
      case Initializer.alive?() do
        ...
        false -> connections ++ publisher ++ [Initializer.child_spec(false)]
      end
    ...
    children =
      case {Config.auto_start?(), Config.disable_worker?()} do
        {true, false} ->
          children ++ [WorkerSupervisor.child_spec(wsv_name)]
